...
Never expose `SUPABASE_SECRET_KEY`, `ADMIN_IMPORT_SECRET`, or `CRON_SECRET` to browser code or commit them to source control.

## 3. Manual import

Restart the app, open `/admin/schedule`, enter `ADMIN_IMPORT_SECRET`, choose a date range, and fetch candidates. Review the parsed schedules and attraction operations before publishing selected rows.
