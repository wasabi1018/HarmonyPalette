alter table public.articles
  add column if not exists destination text not null default 'articles';

alter table public.articles
  drop constraint if exists articles_destination_check;

alter table public.articles
  add constraint articles_destination_check
  check (destination in ('articles', 'guide'));

create index if not exists articles_destination_published_idx
  on public.articles (destination, status, published_at desc)
  where deleted_at is null;

drop function if exists public.search_published_articles(text, text, integer, integer);

create or replace function public.search_published_articles(
  search_query text default '',
  filter_tag_slug text default '',
  filter_destination text default 'articles',
...
as $$
  with matches as (
    ...
    where articles.status = 'published'
      and articles.destination = case
        when filter_destination = 'guide' then 'guide'
        else 'articles'
      end
...
revoke all on function public.search_published_articles(text, text, text, integer, integer)
  from public;
