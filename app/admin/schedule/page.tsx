...
export const metadata: Metadata = {
  title: "スケジュール・キャラクター管理",
  description: "イベント、グリーティング、キャラクター台帳を管理する画面です。",
...
export default function AdminSchedulePage() {
  ...
  return (
    <div className="mx-auto max-w-[1240px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <nav aria-label="パンくずリスト" className="mb-3 flex items-center gap-1 text-[11px] font-bold text-ink/40"><Link href="/" className="hover:text-pink">ホーム</Link><ChevronRight size={12} aria-hidden="true" /><Link href="/admin" className="hover:text-pink">管理</Link><ChevronRight size={12} aria-hidden="true" /><span aria-current="page">スケジュール</span></nav>
      <div className="mb-5 rounded-[26px] border border-pink/10 bg-gradient-to-br from-[#fff0f5] via-white to-[#f4f0fb] px-5 py-6 sm:px-8">
        ...
        <p className="mt-2 text-[13px] font-bold leading-6 text-ink/55">イベント、グリーティング、キャラクター台帳を管理し、公開画面へ反映します。</p>
      </div>
      <ScheduleManagerNotice />
      <OfficialBatchImporter config={{ canWrite: config.canWrite, hasPublicKey: config.hasPublicKey, hasSecretKey: config.hasSecretKey }} hasAdminSecret={Boolean(process.env.ADMIN_IMPORT_SECRET)} />
      <CharacterOrderManager hasAdminSecret={Boolean(process.env.ADMIN_IMPORT_SECRET)} />
      <ScheduleJsonImporter />
      <ScheduleManager hasAdminSecret={Boolean(process.env.ADMIN_IMPORT_SECRET)} />
    </div>
  );
}
